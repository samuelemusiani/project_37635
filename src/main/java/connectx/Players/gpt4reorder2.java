...
/*
 * L9 copy with gpt4 heuristic + move reodering (but better)
 */
public class gpt4reorder2 implements CXPlayer {
  ...
  private Integer[] reorderMoves(CXBoard B) {
    // In the beginning of the game we prefer the center over the evaluation
    if (B.numOfMarkedCells() < Math.sqrt(columns * rows)) {
      ...
      for (int i = 0; i < l; i++) {
        ...
        long position = convertPosition(B);
        Integer val = table.get(position);
        tmp[i] = new intPair(val == null ? 0 : val, m[i]);
        B.unmarkColumn();
      }

      // Sort the array of intPair objects in descending order based on their
      // first value
      Arrays.sort(tmp, new Comparator<intPair>() {
        @Override
      ...
      // System.err.print("(" + tmp[i].first + ", " + tmp[i].second + "), ");
      // }
      // System.err.println("]");

      // Create the output array and copy the columns from the sorted
      // array of intPair objects
      Integer[] r = new Integer[l];
      for (int i = 0; i < l; i++) {
        r[i] = tmp[i].second;
      }
