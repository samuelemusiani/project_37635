...
/*
 * gpt4 copy without transposition table to test large boards collisons
 * + new positional heuristic to play large boards
 * (Does not work well but i keep it)
 */
public class L10 implements CXPlayer {
  ...
  public void initPlayer(int M, int N, int K, boolean first, int timeout_in_secs) {
    ...
    for (int i = 0; i < rows * 3 / 5; i++) {
      for (int j = columns / 2; j < columns; j++) {
        evaPositionalMatrix[i][j] = (int) ((max_dist - Math.sqrt(Math.pow(j -
            center_x, 2) + Math.pow(i - center_y, 2))));
      }
    }
    ...
    // Preferiamo righe a gruppi di due
    for (int i = 1; i < rows * 6 / 7; i += 2) {
      for (int j = columns * 1 / 4; j < columns * 3 / 4; j++) {
        evaPositionalMatrix[i][j] += 40;
      }
    }

    // Preferiamo colonne a blocchi di 3
    for (int i = columns * 1 / 7; i < columns; i += 3) {
      for (int j = 0; j < rows / 2; j++) {
        int val = 20;
        if (evaPositionalMatrix[j][i] < 40)
          evaPositionalMatrix[j][i] += val + 70;
        else
          evaPositionalMatrix[j][i] += val;
      }
    }

    // Non ci piacciono pedine in alto
    // for (int i = rows * 2 / 3; i < rows; i++) {
    // for (int j = 0; j < columns; j++) {
    // evaPositionalMatrix[i][j] -= 2 * i;
    // }
    // }
  ...
  private int evaluate(CXBoard B) {
    ...
    for (int j = 0; j < B.N; j++) {
      ...
      if (found_player) {
        ...
        if (count + count_free < B.X)
          tmp /= 3;
        sum += tmp * (player != am_i_fist ? 1 : -1) * 0.5;
      }
    }
    ...
    int tmp = (int) Math.sqrt(Math.abs(sum));
    return sum >= 0 ? tmp : -tmp;
  }
